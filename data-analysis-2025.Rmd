...
```{r}
# Facet plot to show band usage vs. percent max

# Reshape first, then filter rows where effort is > 0 for that specific band
velocity_long <- weekly_velocity_efforts %>%
  select(pct_of_max_velocity, pct_V2, pct_V3, pct_V4, pct_V5, pct_V6, pct_V7, pct_V8) %>%
  pivot_longer(
    cols = starts_with("pct_V"),
    names_to = "band",
    values_to = "pct_band_effort"
  ) %>%
  filter(pct_band_effort > 0)  # Only filters the relevant band effort

# Clean up band names for labels
velocity_long$band <- recode(velocity_long$band,
                             pct_V2 = "V2 Band",
                             pct_V3 = "V3 Band",
                             pct_V4 = "V4 Band",
                             pct_V5 = "V5 Band",
...
# Plot
ggplot(velocity_long, aes(x = pct_band_effort, y = pct_of_max_velocity)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "loess", se = TRUE, color = "darkred") +
  facet_wrap(~ band) +
  labs(
    title = "Percent of Max Velocity vs. Band Usage",
    x = "Percent of Total Effort in Band (%)",
...
```{r}
# Predicting Weekly Max Velocity

# Reduce for better modeling
bigs <- bigs %>%
  mutate(
    low_band = (mean_V2 + mean_V3) / 2,
    mid_band = (mean_V4 + mean_V5 + mean_V6) / 3,
    high_band = (mean_V7 + mean_V8) / 2
  )
...
skill <- skill %>%
  mutate(
    low_band = (mean_V2 + mean_V3 + mean_V4) / 3,
    mid_band = (mean_V5 + mean_V6) / 2,
    high_band = (mean_V7 + mean_V8) / 2
  )
...
combo <- combo %>%
  mutate(
    low_band = (mean_V2 + mean_V3) / 2,
    mid_band = (mean_V4 + mean_V5) / 2,
    high_band = (mean_V6 + mean_V7 + mean_V8) / 3
  )
model_combo_simple <- lm(mean_weekly_max_velocity ~ low_band + mid_band + high_band, data = combo)
summary(model_combo_simple)
```
Bigs model:
  The model is statistically significant and explains around 69% of the variation in weekly max velocity. The low band is negatively associated, mid band is positively associated, and high band had the strongest positive affect.
  Higher exposure to high-speed sprinting is linked to higher top speeds in bigs, while low-speed work may hinder performance.

Skill model:
  This model explains 33% of variance in weekly max velo. Low band is negatively associated, mid band is positively associated and high band was not significant.
  Moderate-speed exposure appears most beneficial for skill players, while excessive low-intensity work may reduce top-end performance.
  
Combo model:
  This model explains 36% of variance. Only high band is significant, which had a positive effect.
  
Models are not super strong. Could be because of absolute bands.
Will test later when modeling relative bands. Must use weekly mac velocity and not pct_of_max since relative bands are already based on % of max.
...
model_bigs_simple <- lm(mean_pct_max_velocity ~ low_band + mid_band + high_band, data = bigs)
summary(model_bigs_simple)
...
model_skill_simple <- lm(mean_pct_max_velocity ~ low_band + mid_band + high_band, data = skill)
summary(model_skill_simple)
...
model_combo_simple <- lm(mean_pct_max_velocity ~ low_band + mid_band + high_band, data = combo)
summary(model_combo_simple)
```
This model normalizes weekly top speed as a percentage of each athlete’s all-time maximum, allowing for comparisons across players with different physical ceilings. Model fit improved slightly across all position groups, with R² values increasing (e.g., 0.40 for Bigs). Like the previous model, low-band effort remains a strong negative predictor of weekly peak output. This approach is most useful when evaluating relative effort, consistency, or comparing across groups.
Do not use pct_of_max when modeling for relative bands.
...
# Last code chunk did not store these bands for each athlete and only used it for plotting function
...
relative_bands_data <- Catapult_Session_clean %>%
  filter(
    Date >= as.Date("2024-06-30") & Date <= as.Date("2025-07-01"),
    Maximum.Velocity != 0,
    !is.na(Maximum.Velocity)
  ) %>%
   mutate(
    week = floor_date(Date, unit = "week"),
    pct_of_max = (Maximum.Velocity / Player.Max.Velocity) * 100,
...
weekly_max_pct <- Catapult_Session_clean %>%
  ...
  group_by(anon_id, week) %>%
  summarise(
    pct_of_max_velocity = max(pct_of_max, na.rm = TRUE),
    weekly_max_velocity = max(Maximum.Velocity, na.rm = TRUE),
    .groups = "drop"
  )

# Join
relative_bands_data_with_max <- relative_bands_data %>%
  left_join(weekly_max_pct, by = c("anon_id", "week"))

# Join to get positions
relative_bands_data_final <- relative_bands_data_with_max %>%
  left_join(player_positions, by = "anon_id")

# Position groups
relative_bands_data_final <- relative_bands_data_final %>%
  mutate(
    Position_Group = case_when(
      Primary.Position %in% c("QB", "LB", "TE", "RB") ~ "COMBO",
      Primary.Position %in% c("OL", "DL", "DE") ~ "BIGS",
...
relative_bands_group_data <- relative_bands_data_final %>%
  group_by(week, Position_Group) %>%
  summarise(
    mean_V2 = mean(`V2 (20-40%)`, na.rm = TRUE),
    mean_V3 = mean(`V3 (40-50%)`, na.rm = TRUE),
    mean_V4 = mean(`V4 (50-60%)`, na.rm = TRUE),
    mean_V5 = mean(`V5 (60-70%)`, na.rm = TRUE),
    mean_V6 = mean(`V6 (70-80%)`, na.rm = TRUE),
    mean_V7 = mean(`V7 (80-90%)`, na.rm = TRUE),
    mean_V8 = mean(`V8 (90-100%)`, na.rm = TRUE),
    mean_weekly_max_velocity = mean(weekly_max_velocity, na.rm = TRUE),
    mean_pct_max_velocity = mean(pct_of_max_velocity, na.rm = TRUE),
    n_players = n()
  ) %>%
...
```{r}
sprint_exposure_big <- relative_bands_data_final %>%
  filter(Position_Group == "BIGS") %>%
  group_by(week) %>%
...
sprint_exposure_combo <- relative_bands_data_final %>%
  filter(Position_Group == "COMBO")  %>%
  group_by(week) %>%
  mutate(avg_pct_max = mean(pct_of_max_velocity),
         avg_V8 = mean(`V8 (90-100%)`),
...
sprint_exposure_skill <- relative_bands_data_final %>%
  filter(Position_Group == "SKILL")  %>%
  group_by(week) %>%
  mutate(avg_pct_max = mean(pct_of_max_velocity),
         avg_V8 = mean(`V8 (90-100%)`),
...
```{r}
# Model based on mean weekly max velocity

relative_model <- lm(mean_weekly_max_velocity ~ mean_V2 + mean_V4 + mean_V5 + mean_V6 + mean_V7 + mean_V8, data = relative_bands_group_data)
summary(relative_model)

absolute_model <- lm(mean_weekly_max_velocity ~ mean_pct_V2 + mean_pct_V4 + mean_pct_V5 + mean_pct_V6 + mean_pct_V7 + mean_pct_V8, data = weekly_band_effort_by_group)
summary(absolute_model)

bigs_relative <- relative_bands_group_data %>%
  filter(Position_Group == "BIGS")
relative_model_bigs <- lm(mean_weekly_max_velocity ~ mean_V2 + mean_V4 + mean_V5 + mean_V6 + mean_V7 + mean_V8, data = bigs_relative)
summary(relative_model_bigs)

skill_relative <- relative_bands_group_data %>%
  filter(Position_Group == "SKILL")
relative_model_skill <- lm(mean_weekly_max_velocity ~ mean_V2 + mean_V4 + mean_V5 + mean_V6 + mean_V7 + mean_V8, data = skill_relative)
summary(relative_model_skill)

combo_relative <- relative_bands_group_data %>%
  filter(Position_Group == "COMBO")
relative_model_combo <- lm(mean_weekly_max_velocity ~ mean_V2 + mean_V4 + mean_V5 + mean_V6 + mean_V7 + mean_V8, data = combo_relative)
summary(relative_model_combo)

# ADD SAME MODEL techniques as absolute bands

bigs_absolute <- weekly_band_effort_by_group %>%
  filter(Position_Group == "BIGS")
absolute_model_bigs <- lm(mean_weekly_max_velocity ~ mean_pct_V2 + mean_pct_V4 + mean_pct_V5 + mean_pct_V6 + mean_pct_V7 + mean_pct_V8, data = bigs_absolute)
summary(absolute_model_bigs)

skill_absolute <- weekly_band_effort_by_group %>%
  filter(Position_Group == "SKILL")
absolute_model_skill <- lm(mean_weekly_max_velocity ~ mean_pct_V2 + mean_pct_V4 + mean_pct_V5 + mean_pct_V6 + mean_pct_V7 + mean_pct_V8, data = skill_absolute)
summary(absolute_model_skill)

combo_absolute <- weekly_band_effort_by_group %>%
  filter(Position_Group == "COMBO")
absolute_model_combo <- lm(mean_weekly_max_velocity ~ mean_pct_V2 + mean_pct_V4 + mean_pct_V5 + mean_pct_V6 + mean_pct_V7 + mean_pct_V8, data = combo_absolute)
summary(absolute_model_combo)
```
...
No real insights can be made from looking at relative velocity bands and injury occurrences. 
...
week_prior <- injuries_and_running_clean %>%
  ...
  mutate(
    ...
    # Average weeks
    avg_last3weeks = (lag1_pct_of_max_velocity + lag2_pct_of_max_velocity + lag3_pct_of_max_velocity) / 3,
...
# Get Means
means_injury <- injury_week_prior %>%
  drop_na(avg_last3weeks)
avg_last3weeks_mean <- mean(means_injury$avg_last3weeks)

mean_noninjury <- non_injury_week_prior %>%
  drop_na(lag1_pct_of_max_velocity)
non_injury_week_prior_mean <- mean(mean_noninjury$lag1_pct_of_max_velocity)
# Get Means
means_noninjury <- non_injury_week_prior %>%
  drop_na(avg_last3weeks)
avg_last3weeks_mean_non <- mean(means_noninjury$avg_last3weeks)
...
# Plot of % Max Velocity during injury and non injury weeks
ggplot(week_prior, aes(x = week, y = avg_last3weeks)) +
  geom_point(aes(color = factor(injury))) +
  scale_color_manual(values = c("0" = "black", "1" = "red"),
                     labels = c("No Injury", "Injury"),
                     name = "Injury Status") +
  labs(
    title = "Average Percent of Max Velocity Over Last Three Weeks",
    subtitle = paste("Mean % of Max Velocity, Injury: ", round(avg_last3weeks_mean, 2), " |  Mean % of Max Velocity, Non-Injury: ", round(avg_last3weeks_mean_non, 2)),
    x = "Week",
    y = "% of Max Velocity Reached"
  ) +
  theme_classic()
...
```{r}
# Take graph from question 1 and add when injuries happen
...
ggplot(weekly_90_counts, aes(x = week, y = weekly_hits)) +
  ...
  geom_smooth(method = "loess", se = FALSE, color = "#565A5C", linetype = "dashed", linewidth = 0.6) +
  # Injury markers
  geom_vline(data = injury_weeks, aes(xintercept = week),
...
```{r}
# Week of
# Bar chart of sprint counts the week of an injury and bar chart of injruy rate
...
injury_rate_table <- weekly_90pct_injuries %>%
  ...
  summarise(
    ...
    injury_rate = injury_weeks / total_weeks * 100,  # Percentage injured
...
ggplot(injury_rate_table, aes(x = factor(count_90pct), y = injury_rate)) +
  geom_col(fill = "#CFB87C") +
  geom_text(aes(label = injury_rate), vjust = -0.3, size = 4) +
  labs(
    x = "≥ 90% Sprint Count",
    y = "Injury Rate",
    title = "Injury Rate by ≥ 90% Sprint Count in the Week of Injury"
  ) +
  theme_classic()
```
...
injury_table <- matrix(c(
  9, 1248,   # Low (0-1): injured, not injured
  8, 692,     # Moderate (2-3): injured, not injured
  1, 72      # High (3-5): injured, not injured
), nrow = 3, byrow = TRUE)
...
print(fisher_result)
```
Our p-value of 0.32 means that there is no statically significance association between our sprint load groups and injury. 
Injury events are pretty rare overall in our dataset, making it hard to detect subtle differences.


```{r}
# Bar chart of total sprint counts from last three weeks

# Summarize injuries and total weeks by total sprint counts over last 2 weeks
injury_summary_3week <- weekly_90pct_injuries %>%
  mutate(sum_last3weeks = replace_na(sum_last3weeks, 0)) %>%
  group_by(sum_last3weeks) %>%
  summarise(
  ...
  arrange(desc(injury_rate))
injury_summary_3week

ggplot(injury_summary_3week, aes(x = factor(sum_last3weeks), y = total_injuries)) +
  geom_col(fill = "#CFB87C") +
  geom_text(aes(label = total_injuries), vjust = -0.3, size = 4) +
  labs(
    x = "≥ 90% Sprint Count",
    y = "Total Injuries",
    title = "Injuries by Total ≥ 90% Sprint Count Over Last 3 Weeks"
  ) +
  theme_classic()


ggplot(injury_summary_3week, aes(x = factor(sum_last3weeks), y = injury_rate)) +
  geom_col(fill = "#CFB87C") +
  geom_text(aes(label = injury_rate), vjust = -0.3, size = 4) +
  labs(
    x = "≥ 90% Sprint Count",
    y = "Total Injuries",
    title = "Injury Rate by Total ≥ 90% Sprint Counts Over last 3 weeks"
  ) +
  theme_classic()
```
...
# Add row and column names for clarity
rownames(injury_table) <- c("5", "3", "1", "4", "2", "0", "6", "7", "8", "9")
colnames(injury_table) <- c("Injured", "Not_Injured")
...
```{r}
#taking all present IDs
catapult_ids <- unique(Catapult_Session_clean$anon_id)

#calculating each person's weeks
Catapult_Session_clean <- Catapult_Session_clean %>%
  group_by(anon_id) %>%
  mutate(week = floor_date(Date, unit="week")) %>%
  ungroup() %>%
  group_by(anon_id, week) %>%
  #calculating each week's maximum velocity
  mutate(Week.Max.Velocity = mean(na.omit(Session.Max.Velocity)))

#empty vectors for the loop to fill
kendalls <- rep(NA, 104)
negative_trend <- "0"

for(i in 1:104){ #go through all IDs
  ...
  #calculate their kendall correlation coefficient
  kend <- cor(weeks, speeds, method="kendall")
  
  if(length(speeds)>5){ #if there is enough data
  kendalls[i] = kend #save the kendall value
  
    if(kend <= -0.5){
      negative_trend <- c(negative_trend, catapult_ids[i]) #if there is a significant negative trend, store in the vector 
    }
...
```{r}
#looking at the correlations, and those outside the lines are significant
hist(kendalls)
abline(v=-0.5)
abline(v=0.5) 
...
#Splitting up the data sets into injured and uninjured groups and calculating player variance and measurement absolute value
...
#making plot for the difference in pooled variance
...
#plotting bootstrapped group variances
...
#plotting difference in average player variance from bootstrap
...
#plotting player variances from bootstrap
...
#plotting difference in average absolute value
...
#plotting each groups bootstrapped average absolute distance
...
```{r Looking at Running Imbalance for each position category}
for(i in 1:3){ #going through all of the categories for positions
  p <- ggplot(data=Historical_Running_clean[Historical_Running_clean$Position==Positions[i],],aes(Running.Imbalance)) + #looking at distribution of running imbalance
    geom_histogram() +
...
```{r}
#only looking at non repeat values for each player
#don't overcount one person's variance
Player_Summary_Stats <- distinct(Historical_Running_clean[,c("anon_id",
                                                             "Position",
                                                 "Specific.Position", 
                                                 "Player.Variance")])
```
...
```{r}
for(i in 1:3){#looking at player variance grouped by position category
...
#making empty data frame for the bootstrap
...
for(i in 1:5000){
  ...
  #take sample from each category
...
```{r}
#plot the histogram for Combos with 90% CI
...
#plot the histogram for skills with 90% CI 
...
#plot the histofram for bigs with 90% CI
...
```{r}
#storing the confidence intervals in different data frame for plotting
...
#plotting the CIs from bootstrap
...
```{r}
#Making column for absolute value running imbalance
Historical_Running_clean <- Historical_Running_clean %>%
...
#empty data frame for the bootstrap
...
for(i in 1:5000){
  ...
  #taking a sample from each of the position categories
  ...
  #calculating the mean average absolute value running imbalance
...
```{r}
#plotting average absolute distance for each of the categories
...
```{r}
#holding 90% CI for each group in a data frame
...
#plotting each confidence interval with a line chart
...
#looking at prevalence of position categories on entire team
...
```{r}
for(i in 1:22){
  #looking at running imbalance in the weeks leading up to and after HSI
...
for(i in 1:71){
  ...
  if(nrow(Historical_Running_clean[Historical_Running_clean$anon_id == all_IDs[i],])>15){
    ...
    #storing the Kendall correlation coefficient for all of the players
...
```{r}
trends <- trends %>% #adding in column to signify whether a player had HSI
  mutate(injured = ifelse(ID %in% injured_IDs,1,0))

#plotting all the kendall rank coefficients for injured
ggplot(trends[trends$KC>0 & trends$injured==1,], aes(KC)) +
  geom_histogram()

#plotting all the kendall rank coefficients for non injured
...
```{r}
#separating into two data sets, injured and non injured
...
#empty data frame for the bootstrap to fill
...
```{r}
#plotting the difference in average absolute value kendall coefficient
...
#plotting each groups aberage absolute value kendall coefficient
